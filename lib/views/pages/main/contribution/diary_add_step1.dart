...
class _AddDiaryStep1PageState extends State<AddDiaryStep1Page> {
    ...
    try {
      if (!isend) {
        ...
        final mid = await _con.getclinicData(
            page: page, city_id: city_id!, q: q!,filter:'',per_page: 200.toString());
        setState(() {
          if (isloading) {
            clinic_data = mid;
            isloading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if(initDetail && widget.diary_id != ''&& !isloadingDetail) {
      ...
      for(int i=0; i< 6; i++)
        diaryProperties.questions[i] = diaryDetail.text_questions.length>i ? diaryDetail.text_questions[i].pivot!.answer! : '';
      setState(() {
        initDetail = false;
      });
    }
