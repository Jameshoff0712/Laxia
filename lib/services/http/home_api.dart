...
class HomeApi extends Api {
  ...
  Future<Home_Search_Model> homeSearchData(String q, String perpage) async {
    String? token = await preferenceUtil.getToken();
    final res =
        await Api.get("$apiUrl/search?q=" + q + "&perpage=" + perpage, token);
  ...
  Future<Menu> getMenuData(String per_page, String page, String category_id,
      String clinic_id, String q, String filter, String city_id) async {
    String? token = await preferenceUtil.getToken();
    final res = await Api.get(
        "$apiUrl/menus?per_page=" +
            per_page +
            "&page=" +
            page +
            "&category_id=" +
  ...
  Future<bool> postToogleFavorite(int index, String domain) async {
    ...
    final res = await Api.post(
        "$apiUrl/$domain/" + index.toString() + "/toggleFavorite",
        <String, String>{},
        token);
  ...
  Future<bool> postToogleFollow(int index) async {
    String? token = await preferenceUtil.getToken();
    final res = await Api.post(
        "$apiUrl/patients/$index/toggleFollow", <String, String>{}, token);
    if (res!.data["status"] == 1) {
      return true;
    }
    return false;
  ...
  Future<bool> postToogleLike(int index, String domain) async {
    String? token = await preferenceUtil.getToken();
    final res = await Api.post(
        "$apiUrl/$domain/" + index.toString() + "/toggleLike",
        <String, String>{},
        token);
    if (res!.data["status"] == 1) {
      return true;
  ...
  Future<Question> getQuestionData(
      ...
      String orderby,
      String filter,
      String isanswer) async {
    String? token = await preferenceUtil.getToken();
    final res = await Api.get(
        "$apiUrl/questions?per_page=" +
            per_page +
            "&page=" +
            page +
            "&category_id=" +
  ...
  Future<Comment> getCommentList(
      int index, String domain, String page, String per_page) async {
    String? token = await preferenceUtil.getToken();
    final res = await Api.get(
        "$apiUrl/$domain/" +
            index.toString() +
  ...
  Future<Comment_Sub_Model> postComment(
      int index, String domain, String comment, String? parent_id) async {
    String? token = await preferenceUtil.getToken();
    final res;
    if (parent_id == null) {
      res = await Api.post("$apiUrl/$domain/" + index.toString() + "/comments",
          <String, String>{"comments[comment]": comment}, token);
    } else {
      res = await Api.post(
          "$apiUrl/$domain/" + index.toString() + "/comments",
          <String, String>{
